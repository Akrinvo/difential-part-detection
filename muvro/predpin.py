...
# pinparam={'min1': 24, 'max1': 849, 'min2': 40, 'max2': 737, 'min3': 17, 'max3': 483, 'min4': 78, 'max4': 501}
...
def pred_pins(image):
    ...
    for r,j in zip(pin,range(1,5)):
        ...
        if pinparam[f"min{j}"]>pin_black :
            ...
            count+=1
            img1=cv2.rectangle(img1,(r[0],r[1]),(r[0]+r[2],r[1]+r[3]),(0,0,255),3)
            overlay = np.zeros_like(img1)
            overlay[:] = (0, 0,255)
            
        else:
            img1=cv2.rectangle(img1,(r[0],r[1]),(r[0]+r[2],r[1]+r[3]),255,2)
    ...
    img=cv2.putText(img1,str(count),(img1.shape[0]-1,50),cv2.FONT_HERSHEY_COMPLEX,2,(255,0,255),2)
    if count>0:
            img1=cv2.putText(img1,"NG",(20,50),cv2.FONT_HERSHEY_COMPLEX,2,(0,0,255),2)
            img1=cv2.rectangle(img1,(r[0],r[1]),(r[0]+r[2],r[1]+r[3]),(0,0,255),3)
            overlay = np.zeros_like(img1)
            overlay[:] = (0, 0,255)
    else:
        img1=cv2.putText(img1,"OK",(20,50),cv2.FONT_HERSHEY_COMPLEX,2,(0,0,255),2)
        img1=cv2.rectangle(img1,(r[0],r[1]),(r[0]+r[2],r[1]+r[3]),255,2)
        out=1
        overlay = np.zeros_like(img1)
        overlay[:] = (0, 255,0)
